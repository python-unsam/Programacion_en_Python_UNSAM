[Contenidos](../Contenidos.md) \| [Próximo (2 Más sobre funciones)](02_302More_functions.md)
...
En esta sección vemos mas de cerca la escritura de scripts de Python
### Qué es un Script?

Un *script* es un programa que ejecuta una serie de comandos y termina.
...
Se trata a la funciones como ladrillos
Los ladrillos simples ó mas pequeños se definen primero.
...
Las funciones complejas se basan en funciones mas simples, definidas antes; aunque esto es sólo una cuestión de estilo en Python. Lo único que realmente importa en ése programa es que la llamada a `spam(42)` esté al final.

### Diseño de Funciones

Lo ideal es que una función sea una *caja negra*.
Deberían operar únicamente sobre los parámetros provistos, evitar variables globales y efectos secundarios misteriosos. Lo importante es *Diseño Modular* y *Predecibilidad*.

### Doc-Strings

Es buena costumbre incluír documentación en forma de doc-strings. Un doc-string ó "texto de documentación" es texto ubicado en la línea inmediata después del nombre de la función. Proveen información a `help()`, IDEs y otras herramientas.     
...
```python
def leer_precios(filename: str) -> dict:
    '''
...
En la sección 2, escribiste un programa llamado `report.py` que imprime un informe mostrando ...  
El programa tiene algunas funciones, por ejemplo:
...
Modificá tu programa `report.py` de modo que todas las operaciones principales, incluyendo cálculos e impresión, sean llevados a cabo por una colección de funciones. Más específicamente:
...
Empaquetá la última parte de tu programa en una única función `portfolio_report(portfolio_filename, prices_filename)`.
Hacé que si llamamos a esta función del siguiente modo resulte en la creación de un informe, como antes:

```python
portfolio_report('Data/portfolio.csv', 'Data/prices.csv')
```

En su versión final tu programa será una serie de definiciones de funciones seguidos por un único llamado a la funcion `portfolio_report()` (la cual ejecuta todos los pasos que constituyen tu programa).
...
where you just have a file with a sequence of comandos in it. In the
